...
JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    // If the object should be rendered as a div
      ...
      this.editjson_textarea.style.width = Math.max(200,Math.floor(window.innerWidth*15/16)) + 'px';  // '300px'
      ...
      this.addproperty_list.style.width = '195px';  // '295px';
      ...
      this.addproperty_input.style.width = '140px';  // '220px';
      ...
      this.addproperty_add.addEventListener('click',function(e) {
        ...
        if(self.addproperty_input.value) {
          ...
          // self.onChange(true); // modified by wayne 2016/3/22, avoid saving null attribute
        }
      });
      ...
      if (this.header.textContent)  // modified by wayne, 2016/3/19
        this.header.appendChild(document.createElement('br'));  // show buttons in next line
      ...
      // Edit JSON Button
      this.editjson_button = this.getButton('JSON','edit','JSON');
      ...
      this.addproperty_button = this.getButton('Properties','edit','Properties');
  ...
  showEditJSON: function() {
    ...
    // TODO: edge detection
    this.editjson_holder.style.left = '2px'; // this.editjson_button.offsetLeft+"px";
  ...
  showAddProperty: function() {
    ...
    // Position the form directly beneath the button
    // TODO: edge detection
    this.addproperty_holder.style.left = '2px'; // this.addproperty_button.offsetLeft+"px";
...
JSONEditor.defaults.editors.array = JSONEditor.AbstractEditor.extend({
  ...
  addRow: function(value, initial) {
    ...
    // add shift class button, added by wayne at 2016/03/21
    var itemSchema = self.rows[i].schema;
    if (itemSchema.type == 'string' && itemSchema.format == 'classname') {
      if(controls_holder) {
        var shiftCls = document.createElement('button');
        shiftCls.innerHTML = 'Shift name';
        shiftCls.setAttribute('data-i',i+''); // when array item removed, all item node will re-created
        shiftCls.addEventListener('click',function(e) {
          e.preventDefault();
          e.stopPropagation();
          
          var i = e.target.getAttribute('data-i')*1;
          var rows = self.getValue(), sName = rows[i];
          if (!sName || typeof sName != 'string') return;
          var bCls = sName.split('-');
          if (bCls.length <= 1) return;
          
          bCls.push(bCls.shift());
          rows[i] = bCls.join('-');
          self.setValue(rows);
          self.onChange(true);
        },false);
        controls_holder.appendChild(shiftCls);
      }
    }
...
JSONEditor.AbstractTheme = Class.extend({
  ...
  getSwitcher: function(options) {
    ...
    switcher.style.marginLeft = '2px';  // '5px';
