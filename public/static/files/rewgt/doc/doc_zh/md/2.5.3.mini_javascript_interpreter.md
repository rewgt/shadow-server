...
调用空间是一层压一层的栈式结构，当前层为 0 层，上一层是 -1 层，再向上是 -2 层。每一层调用空间都绑捆到某个 R 节点，凡使用了 `$for` 指令或 `$` 引用的节点，都新生成一个调用空间，捆绑到当前使用 `$for` 或 `$` 引用的节点，新生成的空间成为当前 0 层。

用 `$for` 开启调用空间在前文已有举例，现再举一个用 '$' 引用开启调用空间的例子（见 demo28）：

```
  <div $=Panel key='top' height='{null}'>
    <div $=Div key='hello' $for='' dual-data='{{"name":"Wayne","lastName":"Chan"}}'>
